...
public class KhachHang_MaGiamGiaDAO implements DAOInterface<KhachHang_MaGiamGia> {
    ...
    public KhachHang_MaGiamGia selectById(String idMaGiamGia) {
        KhachHang_MaGiamGia khmg = null;
        String sql = "SELECT * FROM khachhang_magiamgia WHERE idMaGiamGia = ?";
        try (Connection conn = JDBCUtil.getConnection(); PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setString(1, idMaGiamGia);
            ResultSet rs = ps.executeQuery();
            if (rs.next()) {
                khmg = new KhachHang_MaGiamGia();
                khmg.setIdMaGiamGia(rs.getString("idmagiamgia"));
                khmg.setMaKhachHang(rs.getString("makhachhang"));
                khmg.setSoluongnguoidungdangco(rs.getInt("soluongnguoidungdangco"));
            }
            conn.close();
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public int delete(KhachHang_MaGiamGia t) {
        int ketQua = 0;
        String sql = "DELETE khachhang_magiamgia WHERE idMaGiamGia = ?";
        try (Connection conn = JDBCUtil.getConnection(); PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setString(1, t.getIdMaGiamGia());
            ketQua = ps.executeUpdate();
            conn.close();
        } catch (Exception e) {
            e.printStackTrace();
