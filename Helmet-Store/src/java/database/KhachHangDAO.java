...
public class KhachHangDAO implements DAOInterface<KhachHang> {
    ...
    public int insert(KhachHang t) {
        ...
        try {
            ...
            // B2: Tạo câu SQL
            String sql = "INSERT INTO khachhang(makhachhang, tendangnhap, matkhau,email,hinhavatar ) VALUES(?,?,?,?,?) ";
            PreparedStatement ps = c.prepareStatement(sql);
            ps.setString(1, t.getMaKhachHang());
            ps.setString(2, t.getTenDangNhap());
            ps.setString(3, t.getMatKhau());
            ps.setString(4, t.getEmail());
            ps.setString(5, t.getHinhAvatar());
    ...
    public KhachHang selectByEmail(String inputEmail) {
        KhachHang kh = null;
        try {
            Connection con = JDBCUtil.getConnection();
            ...
            if (rs.next()) {
                ...
                kh = new KhachHang(maKhachHang, tenDangNhap, matKhau, hoVaTen, gioiTinh, ngaySinh,
                        soDienThoai, email, quocTich, diaChiKhachHang, diaChiNhanHang,
                        maXacThuc, thoiGianHieuLucMaXacThuc, trangThaiXacThuc, hinhAvatar, isAdmin);
            }
        } catch (Exception e) {
    ...
    public KhachHang selectByUsername(String username) {
        ...
        try {
            ...
            String sql = " select makhachhang, email from khachhang\n"
                    + "where tendangnhap = ? ";
            ...
            while (rs.next()) {
                String maKhachHang = rs.getString(1);
                System.out.println("makhachhang111" + maKhachHang);
                kh = new KhachHang();
                String email = rs.getString("email");
                kh.setMaKhachHang(maKhachHang);
                kh.setEmail(email);
                System.out.println("makhachhang222" + kh.getMaKhachHang());
            }
    ...
    public int updateNameAndPhone(KhachHang khachHang) {
        int ketQua = 0;
        try {
            Connection con = JDBCUtil.getConnection();

            String sql = " UPDATE khachhang "
                    + " SET hovaten = ?, sodienthoai = ?, quoctich = ?, diachinhanhang = ? "
                    + " WHERE makhachhang = ?";
