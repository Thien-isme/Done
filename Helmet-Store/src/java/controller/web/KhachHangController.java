...
public class KhachHangController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        } else if (hanhdong.equals("forgotpassword")) {
            forgotpassword(request, response);
        } else if (hanhdong.equals("goToPreview")) {
            goToPreview(request, response);
        } else if (hanhdong.equals("previewProduct")) {
            previewProduct(request, response);
        }
    ...
    private void register(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            if (khdao.isUsernameExists(username)) {
                request.setAttribute("errorUsername", "Username is exist. Please check again!");
                error += "Username exist";
            }

            if (khdao.isEmailExists(email) == true) {
                request.setAttribute("emailInvalid", "Email is exist. Please enter other email!");
                error += "Email exist";
            }
            ...
            if (password.equals(confirmpassword) == false) {
                request.setAttribute("errorConfirmPassword", "Password and confirm password do not match. Please check again!");
                error += "errorConfirmPassword";
            }

            if (!EmailValidator.isValidEmail(email)) {
                request.setAttribute("emailInvalid", "Email Invalid. Please check again!");
                error += "emailInvalid";
            }
    ...
    private void updateProfile(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (EmailValidator.isValidEmail(email) == false) {
            ...
            request.setAttribute("EmailValidator", "Please enter Email valid!");
        }
    ...
    private void resetPassword(HttpServletRequest request, HttpServletResponse response) {
        try {
            ...
            if (password.equals(khachHang.getMatKhau()) == false) {
                request.setAttribute("passpresent", "Password present are not correct.");
                error += "password hiện tại không đúng";
            }

            if (!newPassword.equals(newPasswordConfirm)) {
                request.setAttribute("passConfirm", "NewPassword and NewPasswordConfirm do not match.");
...
//            request.setAttribute("khachHang", khachHang);
            request.setAttribute("prevaction", "resetpassword");
            RequestDispatcher rd = getServletContext().getRequestDispatcher(url);
            rd.forward(request, response);
    ...
    private void viaemailbeforeresetpassword(HttpServletRequest request, HttpServletResponse response) {
        try {
            ...
            if (prevaction.equals("resetpassword")) {
                obj = session.getAttribute("khachHang");
            } else {
                obj = session.getAttribute("khachHang1");
            }
...
//            System.out.println("TimeKahchHnag: " +khachHang.getThoiGianHieuLucMaXacThuc());
            ...
            if (error.length() > 0) {
                ...
                newPasswordtemp = MaHoa.toSHA1(newPasswordtemp);
                khachHang.setMatKhau(newPasswordtemp);
                dao.updateNewPassword(khachHang);

                if (prevaction.equals("resetpassword")) {
                    url = "/khachhang/resetpassword.jsp";
                    request.setAttribute("resetsuccess", "The password has been changed successfully.");
                } else {
                    request.setAttribute("resetsuccess", "The password has been changed successfully.");
                    url = "/khachhang/login.jsp";
                }
...
//        Order order = (Order) session.getAttribute("order");
            ...
            if (order.getList().size() == 0) {
                request.setAttribute("emptyOrder", "Shelf is empty!");
                request.getRequestDispatcher("khachhang/cart.jsp").forward(request, response);
                return;
            }
...
//            request.getRequestDispatcher("/GUI/index.jsp").forward(request, response);
    ...
    private void forgotpassword(HttpServletRequest request, HttpServletResponse response) {
        try {
            ...
            KhachHang khachHang = khd.selectByUsername(username);
            if (khachHang == null) {
                request.setAttribute("usernamenotfound", "The username has not been registered.");
            ...
            if (error.length() > 0) {
                url = "/khachhang/forgotpassword.jsp";
            } else {
                url = "/khachhang/viaemailbeforeresetpassword.jsp";
...
//                khachHang.setMatKhau(newPassword);
            ...
            session.setAttribute("khachHang1", khachHang);
            request.setAttribute("email", khachHang.getEmail());
            request.setAttribute("prevaction", "fogotpassword");
            RequestDispatcher rd = getServletContext().getRequestDispatcher(url);
            rd.forward(request, response);
    ...
    private void update(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        KhachHang newKhachHang = dao.selectById(kh.getMaKhachHang());

        session.setAttribute("khachHang", newKhachHang);
        request.getRequestDispatcher("/khachhang/update.jsp").forward(request, response);
    }

    private void goToPreview(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        HttpSession session = request.getSession(false);
        String madonhang = request.getParameter("orderID");
        ...
        DanhGiaSanPhamDAO dao = new DanhGiaSanPhamDAO();
        int check = dao.kiemTraDonHangDaDuocDanhGiaHayChua(madonhang);
    ...
    private void previewProduct(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
//        HttpSession session = request.getSession(false);
//        List<OrderDetails> listOderDetails = (List<OrderDetails>) session.getAttribute("listOderDetails");
//        for (OrderDetails listOderDetail : listOderDetails) {
//            System.out.println(listOderDetail.getSanPham().getMasanpham()); 
//        }
        HttpSession session = request.getSession(false);
        KhachHang kh = (KhachHang) session.getAttribute("khachHang");
        ...
        List<DanhGiaSanPham> listdanhgiasanpham = new ArrayList<DanhGiaSanPham>();
        int i = 0;
        while (true) {
            i++;
            String masanpham = request.getParameter("masanpham" + i);
            String danhgia = request.getParameter("danhgia" + i);
            String chitiet = request.getParameter("chitiet" + i);

            if (masanpham == null) {
                break;
            } else {
                DanhGiaSanPham danhGiaSanPham = new DanhGiaSanPham();
                danhGiaSanPham.setMaKhachHang(kh.getMaKhachHang());
                danhGiaSanPham.setMaSanPham(masanpham);
                danhGiaSanPham.setDanhGiaSao(Integer.parseInt(danhgia));
                danhGiaSanPham.setChiTiet(chitiet);
                danhGiaSanPham.setTrangthaidanhgia(1);
                danhGiaSanPham.setMaDonHang(madonhang);
                
                DanhGiaSanPhamDAO dao = new DanhGiaSanPhamDAO();
                dao.insert(danhGiaSanPham);
                listdanhgiasanpham.add(danhGiaSanPham);
                System.out.println(danhGiaSanPham.toString());
            }
        }
        ...
        OrderHistoryDAO dao = new OrderHistoryDAO();
        list = dao.selectByIdMaKhachHang(kh.getMaKhachHang());
